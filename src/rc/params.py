...
def get_initial_conditions(stages: List[Stage]) -> List[Telemetry]:
    ...
    telemetries = [Telemetry(num_timesteps, stage.mass_prop, position_0, velocity_0, acceleration_0,
                             barometric_density(0.0), 0.0) for stage in stages]
    return telemetries
...
def get_downranges(phis: List[float]) -> List[float]:
